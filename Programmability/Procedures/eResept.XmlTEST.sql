...
GO
CREATE PROCEDURE [eResept].[XmlTEST] (
...
BEGIN
  ...
  DECLARE @DrugTreatmentXml TABLE (XML XML NOT NULL);

  INSERT INTO @DrugTreatmentXml
  VALUES (CONVERT(XML, @XmlString));

  DECLARE @DrugTreatment TABLE (LibId VARCHAR(40) NOT NULL,
  ...
  INSERT INTO @DrugTreatment (LibId,
                              DrugName,
                              Varenr,
                              StartAt,
                              StartReason,
                              ...
                              Seponeringskladd,
                              TreatType)
  SELECT NULLIF(CAST(c.query('data(../LibId)') AS VARCHAR(40)), '') AS LibId,
         ...
         CASE
                       ...
                       ELSE TRY_CONVERT(DATETIME, CAST(c.query('data(../InstitueringsDato)') AS VARCHAR(MAX))) END AS StartAt,
         ...
         CASE
                       ...
                       ELSE TRY_CONVERT(DATETIME, CAST(c.query('data(../SeponeringsInfomasjon/Seponeringstidspunkt)') AS VARCHAR(MAX))) END AS Seponeringstidspunkt,
         CASE
                       WHEN
                         CAST(c.query('data(../SeponeringsInfomasjon/Seponeringsdato)') AS VARCHAR(MAX)) = '' THEN NULL
                       ELSE TRY_CONVERT(DATETIME, CAST(c.query('data(../SeponeringsInfomasjon/Seponeringsdato)') AS VARCHAR(MAX))) END AS Seponeringsdato,
  ...
  DELETE FROM @DrugTreatment
   WHERE Varenr NOT BETWEEN 601 AND 604; -- Magic numbers

  -- Extract from ReseptDokLegemiddel
  INSERT INTO @DrugTreatment (InteraksjonsNiva,
                              InteraksjonsKommentar,
                              LibId,
                              LegemiddelpakningATC,
                              LegemiddelVirkestoffATC,
                              DrugName,
                              DrugForm,
                              StrengthStr,
                              Varenr,
                              ...
                              VarselSlvTypeDN,
                              VarselSlvOverskrift,
                              VarselSlvVarseltekst)
  SELECT CONVERT(INT, CAST(c.query('data(../InteraksjonsInformasjon/InteraksjonsNiva/@V)') AS VARCHAR(10))) AS InteraksjonsNiva,
  ...
  -- Add StopBy
  MERGE INTO @DrugTreatment Trg
  USING (SELECT UserId, FMUserName FROM dbo.UserList) Src
     ON (Trg.SeponertAv = Src.FMUserName)
  WHEN MATCHED THEN UPDATE SET Trg.StopBy = Src.UserId;

  -- Add CreatedBy
  MERGE INTO @DrugTreatment Trg
  USING (SELECT UserId, FMUserName FROM dbo.UserList) Src
     ON (Trg.RegistrertAv = Src.FMUserName)
  WHEN MATCHED THEN UPDATE SET Trg.RegistrertAvUserId = Src.UserId;
...
-- Varenummer som begynner med 601-604, FIB uinteressant, Vaksiner uinteressante.
  ...
  UPDATE @DrugTreatment
     SET RxText = REPLACE(REPLACE(REPLACE(RxText, '&lt;','<'),'&gt;','>'), '&amp;','&');

  SELECT * FROM @DrugTreatment;
END
GO