...
GO
CREATE PROCEDURE [Dash].[NdvType1UntreatedLDLLe25] AS
BEGIN
    ...
    (
      ...
      (
        ...
        WHERE p.DOB >= MonthList.FirstDOB AND p.DOB < MonthList.LastDOB 
          AND ISNULL( v.Treated, 2 ) = 2 AND ISNULL( v.HasCVD, 0 ) = 0 
      ) Type1Patients
...
GRANT EXECUTE ON [Dash].[NdvType1UntreatedLDLLe25] TO [FastTrak]
GO