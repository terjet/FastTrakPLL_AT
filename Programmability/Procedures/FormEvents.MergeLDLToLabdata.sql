...
GO
CREATE PROCEDURE [FormEvents].[MergeLDLToLabdata] ( @ClinFormId INT) AS
BEGIN
  ...
  -- Merge new data from the form into labdata
  ...
  ( 
    ...
    WHERE cf.ClinFormId = @ClinFormId AND cdp.ItemId = 1884 AND Quantity > 0
  ) src
...
GRANT EXECUTE ON [FormEvents].[MergeLDLToLabdata] TO [FastTrak]
GO