...
BEGIN
  ...
  DECLARE @DrugReactions TABLE (
    ...
    VirkestoffIdListe VARCHAR(MAX) NULL,
    VirkestoffNavnListe VARCHAR(MAX) NULL,
  ...
  ATCKodeV, ATCKodeDN, VirkestoffIdListe, VirkestoffNavnListe, VirkestoffIdAlt, VirkestoffNavnAlt, ReaksjonDN, ReaksjonV, KildeV, KildeDN, AlvorlighetsgradV,
  ...
  WHEN MATCHED
    THEN UPDATE
      ...
      Trg.VirkestoffId = NULLIF(Src.VirkestoffIdListe, ''),
      Trg.VirkestoffNavn = Src.VirkestoffNavnListe,
  ...
  PRINT ' Iterate over @DrugReactions to create a list of CaveId that form CAVE details for a particular drug treatment.';

  DECLARE treat_cur CURSOR FAST_FORWARD FOR
  SELECT b.TreatId, COALESCE(a.VirkestoffNavnListe, a.LegemiddelNavn) + ': ' + a.GrunnlagForCAVE AS GrunnlagForCAVE
  FROM @DrugReactions a
